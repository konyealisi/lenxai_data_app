...
layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            dcc.Dropdown(id='lga-filter2',
                        options=[{'label': i, 'value': i} for i in (['All'] + list(merged_df['lga'].unique()))],
                        value=['All'],
                        multi=True)], width={"xs": 6, "sm": 4, "md": 2}),
        dbc.Col([
            html.Label("Facility", style={'color': 'white'}),
            dcc.Dropdown(id='facility-filter2',
                        options=[{'label': i, 'value': i} for i in (['All'] + list(merged_df['facility_name'].unique()))],
                        value=['All'],
                        multi=True)], width={"xs": 6, "sm": 4, "md": 2}),
        dbc.Col([
            html.Label("Sex", style={'color': 'white'}),
            dcc.Dropdown(id='sex-filter2',
                        options=[{'label': i, 'value': i} for i in (['All'] + list(merged_df['sex'].unique()))],
                        value=['All'],
                        multi=True)], width={"xs": 6, "sm": 4, "md": 2}),
        dbc.Col([
            html.Label("Age Group", style={'color': 'white'}),
            dcc.Dropdown(id='age-group-filter2',
                        options=[{'label': i,'value': i} for i in (['All'] + ["<1", "1-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65+"])],
                        value=['All'],
                        multi=True)], width={"xs": 6, "sm": 4, "md": 2}),
    ]),
    dbc.Row([
        dbc.Col(html.H3("Validation Analytics", className='text-center'), className="mb-4 mt-4", style={'color': 'white'})
    ]),
     dbc.Row([
        dbc.Col(dcc.Graph(id='pr_ndr'), width=12),
    ]),
    html.Br(),
    dbc.Row([
        dbc.Col(dcc.Graph(id='pr_cr'), width=12),
    ]),
    html.Br(),
    dbc.Row([
        dbc.Col(dcc.Graph(id='cr_ndr'), width=12),
    ]),
    # html.Br(),
    # dbc.Row([
    #     dbc.Col(dcc.Graph(id='vf_chart01'), width=6),
    #     dbc.Col(dcc.Graph(id='vf_chart11'), width=6),
    # ]),
    # html.Br(),
    # dbc.Row([
    #     dbc.Col(dcc.Graph(id='vf_chart21'), width=6),
    #     dbc.Col(dcc.Graph(id='vf_chart31'), width=6),
    # ]),
    # html.Br(),
    # dbc.Row([
